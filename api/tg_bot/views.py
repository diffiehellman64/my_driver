...
def create_file_from_message(bot: TG_DJ_Bot, update: Update, user: User):
    ...
    # fl = FileViewSet(telegram_reverse('tg_bot:FileViewSet'), update=update, user=user, bot=bot)
    # fl.create('folder', root_folder.pk)
    # fl.create('media_id', '')

    vvs = VehicleViewSet(telegram_reverse('tg_bot:VehicleViewSet'), user=user)
    __, (message, buttons) = vvs.show()
